...
public class Enemy : MonoBehaviour, IDamageable
{
    ...
    //     if (Vector3.Distance(transform.position, Player.transform.position) >= FindDistance || Vector3.Distance(transform.position, StartPosition) >= ReturnDistance)
    //     {
    ...
    //
    //     // 시작 위치와되돌아간다
    //     Vector3 diraction = (StartPosition - transform.position).normalized;
    //     // _characterController.Move(diraction * MoveSpeed * Time.deltaTime);
    ...
    //     }
    //     _patrolPoints.Add(StartPosition); // 마지막은 시작 위치
    // }
    //
